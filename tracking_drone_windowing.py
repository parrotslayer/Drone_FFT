...
# initialize portaudio
    ...
    #extract channels
    left_buff = numpydata[1::2]
    right_buff = numpydata[0::2]
    size_buff = len(left_buff)
    
    # Only begin FFT when enough samples collected
    if loop <= 10:
        #generate the first FFT window last 1 second
        left.extend(left_buff)
        right.extend(right_buff)
    else:           
        # Take 9 more recent chunks and put at beginning of array
        left[0:9*size_buff] = left[size_buff:]
        right[0:9*size_buff] = right[size_buff:]
        # Put the most recent data in the last 10th chunk slot
        left[9*size_buff:] = left_buff
        right[9*size_buff:] = right_buff
        ...
        plt.suptitle('Noise Floor Test {0} Loop {1}'.format(testnum,loop-10))
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Amplitude')
        plt.savefig('NF_Test{0}_Loop{1}'.format(testnum,loop-10))
        ...
        if len(peaks_L_amp) > 0 and len(peaks_R_amp) > 0:
            ...
            plt.suptitle('Left and Right Channels Test {0} Loop {1}'.format(testnum,loop-10))
            plt.xlabel('Frequency (Hz)')
            plt.ylabel('Amplitude')
            plt.savefig('LR_Test{0}_Loop{1}'.format(testnum,loop-10))
            #plt.show(block = False)
